...
async function loadAngleFolders() {
  ...
  const select = qs("infer-character");
  select.innerHTML = "";
  ...
  if (state.angleFolders[0]) {
    renderGallery(qs("angle-gallery"), state.angleFolders[0].name, state.angleFolders[0].images);
  }
}
...
async function handleAngleGenerate() {
  ...
  try {
    ...
    renderGallery(qs("angle-gallery"), data.folder, data.images);
